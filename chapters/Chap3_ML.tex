...
Vẫn sử dụng mô hình hệ thống MIMO/mMIMO đã trình bày ở hình~\ref{fig:sys_model}.
% Tuy nhiên, thay vì mô hình hoá tín kênh truyền vô tuyến dưới dạng các bộ lọc FIR có chiều dài $M+1$, trong DNN, giả sử: (i) $T$ có thể coi là số lượng ăng-ten bên phát hoặc số lượng người dùng (user) với mỗi người dùng chỉ có một ăng-ten phát, (ii) ma trận $\mathbf{H}$ là biến đổi tuyến tính của tín hiệu truyền thành tín hiệu nhận được tức $M=0$, (iii) $N=1$ tức mỗi ăng-ten nhận chỉ thu thập một ký hiệu tại một thời điểm, (iv) 
\begin{equation}
...
Kiến trúc DetNet dựa trên phương pháp PGD~\cite{Chen2015} cho việc tối ưu MLE như trên phương trình (\ref{eq:mle}). Đạo hàm riêng được tách như trên (\ref{eq:pgd}) sử dụng luật chuỗi (chain rule)~\cite{Minka2000}.
\begin{equation}
\label{eq:pgd}
    \begin{aligned}
...
Kiến trúc một lớp mạng ISDNN cho kênh truyền có cấu trúc với thông tin bên lề là DoA tại bên thu được biểu diễn như trên hình~\ref{fig:ISD_structured}. Các tham số khởi tạo và quá trình đào tạo của mạng vẫn tương tự như mô hình ISDNN đã được trình bày sử dụng cho mô hình kênh truyền phi cấu trúc đã trình bày tại mục~\ref{sec:ISNN_nonstructured}.
\begin{figure}[ht]
    \centering
    \begin{tikzpicture}
        \node (B11) [field6, fill=red!10!white] at (0, 0) {$\mathbf{e}_k$};
    ...
    \end{tikzpicture}
    \caption{Kiến trúc của một lớp trong mô hình mạng nơ-ron sâu ISDNN đề xuất cho mô hình kênh truyền có cấu trúc. Giả sử biết thông tin của DoA tại bên thu $\boldsymbol{\varphi}(\boldsymbol\theta, \boldsymbol\phi$).}
...
Ma trận kênh truyền $\mathbf{H}$ lấy theo mô hình kênh Rayleigh fading, trong đó, các hệ số phức của kênh truyền được gieo ngẫu nhiên độc lập và cùng phân bố Gaussian đối xứng tròn (circularly symmetric Gaussian) với giá trị trung bình $\mu$ và độ lệch chuẩn $\sigma^2$ như trên hình~\ref{fig:ray_H}.
\begin{equation}
...
\subsubsection{So sánh độ phức tạp của các phương pháp}
...
\subsubsection{So sánh độ chính xác của mô hình đề xuất với các phương pháp khác}
...
\subsubsection{Xem xét độ chính xác của mô hình ISDNN khi có sai số trong tập dữ liệu đào tạo}
