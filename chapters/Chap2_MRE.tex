...
% Sau đó, các hai phương pháp nhằm giảm thiểu chi phí của thuật toán SB-MRE được đề xuất, bao gồm giảm độ phức tạp của B-MRE và giảm thiểu độ dài chuỗi pilot. Cuối cùng, các mô phỏng được thực hiện để kiểm chứng thuật toán đề xuất và đưa ra kết luận.
...
\begin{figure}
    ...
    \begin{tikzpicture}
        ...
        \node at (0, 17mm) (text_Tx0) {Tx $t$};
        \draw[line] (-10mm, 0) -- (Tx0) node [pos=-1, below=3mm] {$[s_t(n), s_t(n-1), \ldots, (s_t(n-K+1)]^\top$};

        \node[antenna, thick, scale=0.6] at (50mm, 0) (Rx1) {};
        \node at (50mm, 17mm) (text_Rx1) {Rx $1$};
        \draw[line] (60mm, 0) -- (Rx1) node [pos=-1, below=3mm] {$[x^{(1)}(n), x^{(1)}(n-1), \ldots, x^{(1)}(n-N+1)]^\top$};

        \node[antenna, thick, scale=0.6] at (50mm, 30mm) (Rx0) {};
        \node at (50mm, 47mm) (text_Rx0) {Rx $0$};
        \draw[line] (60mm, 30mm) -- (Rx0) node [pos=-1, below=3mm] {$[x^{(0)}(n), x^{(0)}(n-1), \ldots, x^{(0)}(n-N+1)]^\top$};

        \node[scale=1.5] at (50mm, -14mm) () {$\mathbf{\vdots}$};

        \node[antenna, thick, scale=0.6] at (50mm, -40mm) (RxL) {};
        \node at (50mm, -23mm) (text_RxL) {Rx $L-1$};
        \draw[line] (60mm, -40mm) -- (RxL) node [pos=0, below=3mm] {$[x^{(L-1)}(n), x^{(L-1)}(n-1), \ldots, x^{(L-1)}(n-N+1)]^\top$};

        \draw[arrow] (3mm, 6mm) -- (47mm, 35mm) node [pos=0.5, above=3mm] {$\mathbf{H}_t^{(0)}$};
        \draw[arrow] (3mm, 5mm) -- (47mm, 5mm) node [pos=0.5, above=3mm] {$\mathbf{H}_t^{(1)}$};
        \draw[arrow] (3mm, 4mm) -- (47mm, -35mm) node [pos=0.5, below=4mm] {$\mathbf{H}_t^{(L-1)}$};
    \end{tikzpicture}
    \caption{Minh hoạ hệ thống SIMO sử dụng phương pháp nhận dạng B-MRE.}
    \label{fig:MRE_model}
\end{figure}
...
% , có thể nhận thấy, tín hiệu tại các ăng-ten thu khác nhau có sự phân tập về độ trễ. 
...
\begin{figure}
    ...
    \begin{tikzpicture}[node distance = 0mm]
        \node[field5, minimum height=20mm, minimum width=20mm, fill=red!10!white] (B1) {$s_t(0)$};
        \node[right=-1mm of B1, field5, minimum height=20mm, minimum width=20mm, fill=red!10!white] (B2) {$s_t(1)$};
        \node[right=-1mm of B2, field5, minimum height=20mm, minimum width=20mm, fill=red!10!white] (B3) {$\cdots$};
        \node[right=-1mm of B3, field5, minimum height=20mm, minimum width=20mm, fill=red!10!white] (B4) {$s_t(N_p - 1)$};
        \node[right=-1mm of B4, field5, minimum height=20mm, minimum width=20mm] (B5) {$s_t(N_p)$};
        \node[right=-1mm of B5, field5, minimum height=20mm, minimum width=20mm] (B6) {$s_t(N_p + 1)$};
        \node[right=-1mm of B6, field5, minimum height=20mm, minimum width=20mm] (B7) {$\cdots$};
        \node[right=-1mm of B7, field5, minimum height=20mm, minimum width=20mm] (B8) {$s_t(N_s - 1)$};

        \draw [very thick,decorate,decoration={calligraphic brace,amplitude=10pt}] (-10mm, 12mm) -- (68mm, 12mm) node [pos=0.5, above=3mm] {$N_p$ ký hiệu pilot} ;
        \draw [very thick,decorate,decoration={calligraphic brace,amplitude=10pt}] (69mm, 12mm) -- (149mm, 12mm) node [pos=0.5, above=3mm] {$N_s - N_p$ ký hiệu dữ liệu} ;
    \end{tikzpicture}
    \caption{Minh hoạ chuỗi một ký hiệu $\mathbf{s}_t$ sử dụng cho SB-MRE.}
    \label{fig:mre_sequence}
\end{figure}
